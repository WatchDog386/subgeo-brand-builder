...
import { branchBySlug, branches, type Branch } from "@/lib/branches";

export const Route = createFileRoute("/branches/$slug")({
  loader: ({ params }) => {
    const branch = branchBySlug(params.slug);
  ...
  head: ({ loaderData }) => {
    ...
    if (!b) {
      return { meta: [{ title: "Branch not found — Subgeo" }] };
    }
    return {
      meta: [
        { title: `${b.name} — Subgeo` },
        { name: "description", content: b.tagline },
        { property: "og:title", content: `${b.name} — Subgeo` },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="text-center">
        ...
        <Link to="/branches" className="mt-4 inline-block text-brand">
          Back to branches
        </Link>
      </div>
    </div>
